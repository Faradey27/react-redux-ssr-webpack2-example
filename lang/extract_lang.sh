...
IFS=$'\n' read -r -d '' -a locales < ${ROOT_DIR}/lang/supportedLocales.txt  # read into an array
...
pathToMessages=${ROOT_DIR}/lang/locales/po/messages.pot
...
do
   :
   path=${ROOT_DIR}/lang/locales/po/$locale.po;
   [ -f ${path} ] && msgmerge --backup=off -U "${path}" "${pathToMessages}" ||
   cat ${ROOT_DIR}/lang/locales/base.po > "${path}" && msgmerge --backup=off -U "${path}" "${pathToMessages}";
done
